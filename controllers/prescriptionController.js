...
router.post("/", upload.single("image"), async (req, res) => {
  try {
    ...
    let patient_id = null;
    if (inpatient_fetch.length === 0 && outpatient_fetch.length != 0) {
      patient_id = outpatient_fetch[0]._id;
    } else if (outpatient_fetch.length === 0 && inpatient_fetch.length != 0)
      patient_id = inpatient_fetch[0]._id;
    else patient_id = inpatient_fetch[0]._id;
    let doctor_fetch = await Doctor.find({ phone: doctor });
    let doctor_id = doctor_fetch[0]._id;
    if (patient_id == null || doctor_id == undefined)
      return res.json({ msg: "Invalid Phone numbers" });
...
router.patch("/:id", async (req, res) => {
  try {
    ...
    if (!prescription)
      return res.status(400).json({ msg: "No prescription found" });
    const fields = {};
    if (patient.length != 0) {
      inpatient_fetch = await Inpatient.find({ phone: patient });
      outpatient_fetch = await Outpatient.find({ phoneNo: patient });
    ...
    if (medicine.length != 0) {
      ...
      if (medicine_fetch[0].name != id_med[0].name) {
        if (medicine_fetch === undefined)
          return res.json({ msg: "Invalid Medicine Name" });
        const fields2 = {};
